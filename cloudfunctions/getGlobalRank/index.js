// cloudfunctions/getGlobalRank/index.js
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    try {
      const cacheRes = await db.collection('rank_cache').doc('win_' + weekKey).get();
      if (cacheRes.data && cacheRes.data.list) {
        ...
        fromCache = true;
        console.log('[getGlobalRank] 命中缓存，已匿名化处理');
      }
    } catch (e) {
      console.log('[getGlobalRank] 缓存不存在，回退到原逻辑');
    }
    ...
    if (!fromCache) {
      ...
      if (userIds.length > 0) {
        try {
          ...
          console.error('[getGlobalRank] 获取头像失败:', e);
        }
      }
    ...
    try {
      ...
      if (myStat.data) {
        myScore = (myStat.data.currentWeek === weekKey) ? (myStat.data.weeklyWinCount || 0) : 0;
        ...
        if (myScore > 0) {
          ...
          if (inList) {
            ...
            try {
              ...
              console.log('[getGlobalRank] 计算排名失败:', e);
            }
          }
        }
      }
    } catch (err) {
      console.log('[getGlobalRank] 用户暂无排名数据');
    }
    ...
    console.error('[getGlobalRank] 获取全服榜失败', err);
